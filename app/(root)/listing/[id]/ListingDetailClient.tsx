...
type ListingData = NonNullable<GetListingByIdQuery['getListingById']> & {
  business?: NonNullable<GetListingByIdQuery['getListingById']>['business'] & {
    owner?: { id: string }
    businessUsers?: Array<{ user: { id: string; username: string } }>
  }
}
          ...
          <div className='md:w-1/2 space-y-6 flex flex-col justify-between'>
            <div>
              ...
              <p className='text-sm text-gray-500'>
                ...
                {listing.customCity ||
                  (listing.city
                    ? `${listing.city.name}${listing.city.region ? `, ${listing.city.region.name}, ${listing.city.region.country.name}` : ''}`
                    : '')}
              </p>
            </div>

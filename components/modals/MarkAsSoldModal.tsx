...
export default function MarkAsSoldModal({
  ...
  const [salePrice, setSalePrice] = useState(listing.price.toString() ?? '0')
      ...
      <DialogContent className='max-w-2xl max-h-[90vh] overflow-y-auto'>
        ...
        >
          <div className='bg-popover p-4 rounded-lg'>
            ...
            <p className='text-sm text-gray-300'>{listing.title}</p>
            <p className='text-sm text-gray-300'>
              Original Price: R{listing.price}
            </p>
          </div>
          ...
          <div>
                ...
                {searching ? (
                  ...
                  <div className='divide-y'>
                    {users.map((user: User) => (
                      <div
                        key={user.id}
                        className='p-3 cursor-pointer hover:bg-popover'
                        onMouseDown={(e) => {
                          e.preventDefault()
            ...
            {/* Selected Buyer */}
            {selectedBuyer && (
              <div className='mt-3 p-3 bg-mutedForeground border border-blue-200 rounded-lg'>
                <div className='flex items-center gap-3'>
                  ...
                  <div className='flex-1'>
                    ...
                    {selectedBuyer.trustRating && (
                      <div className='flex items-center gap-1 text-sm  '>
                        <Star className='h-3 w-3 text-yellow-500 fill-current' />
                        <p>{selectedBuyer.trustRating.starRating.toFixed(1)}</p>
                        <Badge
                          variant='outline'
                          className={`ml-1 text-xs ${getTrustLevelColor(

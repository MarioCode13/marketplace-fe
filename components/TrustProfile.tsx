...
import { Shield, CheckCircle, Clock, XCircle, User } from 'lucide-react'
...
import { VerificationDocument, DocumentType, VerificationStatus } from '@/lib/graphql/types/trust'
...
export default function TrustProfile({ userId, user }: TrustProfileProps) {
  ...
  const { data: reviewsData, loading: reviewsLoading } = useQuery(GET_USER_REVIEWS, {
    variables: { userId },
    skip: !userId
  })
  ...
  const { data: positiveReviewCountData } = useQuery(GET_USER_POSITIVE_REVIEW_COUNT, {
    variables: { userId },
    skip: !userId
  })
  ...
  const positiveReviews = positiveReviewsData?.getUserPositiveReviews || []
  const negativeReviews = negativeReviewsData?.getUserNegativeReviews || []
  const averageRating = averageRatingData?.getUserAverageRating || 0
  const reviewCount = reviewCountData?.getUserReviewCount || 0
  const positiveReviewCount = positiveReviewCountData?.getUserPositiveReviewCount || 0
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div className="text-center p-4 bg-green-50 rounded-lg">
              <div className="text-2xl font-bold text-green-600">{positiveReviewCount}</div>
              <div className="text-sm text-gray-600">Positive Reviews</div>
            </div>
            <div className="text-center p-4 bg-red-50 rounded-lg">
              <div className="text-2xl font-bold text-red-600">{reviewCount - positiveReviewCount}</div>
